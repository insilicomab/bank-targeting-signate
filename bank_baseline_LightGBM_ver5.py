...
ver.3.1: clipping(balance)
ver.4: clipping(duration)
ver.5: 対数変換（balance, duration）
"""
...
'''
対数変換（balance, duration）
'''

# balanceの対数変換
df['balance_log'] = np.log(df['balance'] - df['balance'].min() + 1)
plt.hist(df['balance_log'], bins=100)
plt.title('balance_log')
plt.show()

# durationの対数変換
df['duration_log'] = np.log(df['duration'] - df['duration'].min() + 1)
plt.hist(df['duration_log'], bins=100)
plt.title('duration_log')
plt.show()

# 対数変換前のカラムを削除
df = df.drop(['balance', 'duration'], axis=1)
...
# 1%、99%点を計算し、clipping
p01 = df['balance_log'].quantile(0.01)
p99 = df['balance_log'].quantile(0.99)
df['balance_log'] = df['balance_log'].clip(p01, p99)
...
# CSVファイルの出力
sub.to_csv('./submit/bank_baseline_ver5.csv', header=None, index=False)
