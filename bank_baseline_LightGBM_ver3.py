...
data = df['duration'].values.reshape(-1,1)
pt.fit(data)
df['duration'] = pt.transform(data)
...
# campaignのヒストグラム
plt.hist(df['campaign'], bins=50)
plt.title('campaign')
plt.show()

# Box-Cox変換
pt = PowerTransformer(method='box-cox')
data = df['campaign'].values.reshape(-1,1)
pt.fit(data)
df['campaign'] = pt.transform(data)

# Box-Cox変換後のcampaignのヒストグラム
plt.hist(df['campaign'], bins=50)
plt.title('campaign（Box-Cox変換）')
plt.show()
...
# CSVファイルの出力
sub.to_csv('./submit/bank_baseline_ver3.csv', header=None, index=False)
